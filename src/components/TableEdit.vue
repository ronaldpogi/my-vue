<template>
  <el-form ref="formRef" :model="form" :rules="rules" :label-width="options.labelWidth">
    <el-row>
      <el-col :span="options.span" v-for="item in options.list" :key="item">
        <el-form-item :label="item.label" :prop="item.prop">
          <!-- Text input, number input, dropdown, date picker, switch, upload -->
          <el-input
    ...
    <el-form-item>
      <el-button type="primary" @click="saveEdit(formRef)">Save</el-button>
    </el-form-item>
  </el-form>
</template>
...
const rules: FormRules = options.list
  .map((item: any) => {
    if (item.required) {
      return {
        [item.prop]: [
          { required: true, message: `${item.label} cannot be empty`, trigger: 'blur' },
        ],
      }
    }
    return {}
